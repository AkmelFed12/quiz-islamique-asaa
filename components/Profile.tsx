...
import { hasUserTakenQuizToday, calculateProgressiveDifficulty, getQuizRecommendation } from '../services/quizManagementService';
import { User as UserIcon, Award, Calendar, Hash, History, Loader2, Lock, TrendingUp, CheckCircle, Clock } from 'lucide-react';
...
export const Profile: React.FC<ProfileProps> = ({ user }) => {
  ...
  const quizTakenToday = hasUserTakenQuizToday(user.username, results);
  const nextDifficulty = calculateProgressiveDifficulty(user.username, results);
  const recommendation = getQuizRecommendation(user, results);
          ...
          <div className="mt-6 pt-6 border-t border-gray-200">
            {quizTakenToday ? (
              <div className="flex items-center justify-center gap-2 text-emerald-700 font-semibold">
                <CheckCircle size={20} />
                <span>Quiz complété aujourd'hui ✓</span>
              </div>
            ) : (
              <div className="flex items-center justify-center gap-2 text-blue-700 font-semibold">
                <Clock size={20} />
                <span>Quiz disponible - Prochaine difficulté: {nextDifficulty}</span>
              </div>
            )}
          </div>
        </div>
